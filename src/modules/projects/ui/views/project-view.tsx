...
export const ProjectView = ({ projectId }: Props   ) => {
  ...
  const [mobileTab, setMobileTab] = useState<"messages" | "fragment">("messages");
    ...
    <div className="h-screen">
      ...
      <div className="md:hidden h-full flex flex-col">
        <div className="border-b">
          <Tabs value={mobileTab} onValueChange={(v) => setMobileTab(v as "messages" | "fragment") }>
            <TabsList className="flex w-full gap-1 p-1 bg-background/5 rounded-md m-2">
              <TabsTrigger className="flex-1 justify-center py-2 rounded-md text-sm font-medium text-muted-foreground data-[state=active]:bg-primary/10 data-[state=active]:text-primary" value="messages">Messages</TabsTrigger>
              <TabsTrigger className="flex-1 justify-center py-2 rounded-md text-sm font-medium text-muted-foreground data-[state=active]:bg-primary/10 data-[state=active]:text-primary" value="fragment">Fragment</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
              ...
              >
                <div className="w-full flex items-center p-2 border-b gap-x-2">
                  <TabsList className="flex h-9 items-center gap-1 rounded-lg p-1 bg-background/5 mx-2 shadow-sm">
                    <TabsTrigger className="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-muted-foreground data-[state=active]:bg-primary/10 data-[state=active]:text-primary rounded-md" value="preview">
                      <EyeIcon className="w-4 h-4" />
                      Preview
                    </TabsTrigger>
                    <TabsTrigger className="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-muted-foreground data-[state=active]:bg-primary/10 data-[state=active]:text-primary rounded-md" value="code">
                      <CodeIcon className="w-4 h-4" />
                      Code
                    </TabsTrigger>
                  </TabsList>
